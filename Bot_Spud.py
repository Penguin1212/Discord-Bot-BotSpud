...
# When a new member joins what is done (being fixed )
    ...
    async def on_member_join(member):
        ...
        print('Just gave ' + str(member) + " " + str(role))
...
# custome help command
    ...
    async def help(ctx):
        ...
        embed.add_field(name='transfer', value='Give currency to others when you suck ass | aliases = [pay, give]',
                        inline=False)
        embed.add_field(name='roll', value='Roll some dice brotha | roll [1-4] [2-unlimited] | aliases = []',
                        inline=False)
...
# Roll command
    ...
    @bot.command(pass_context=True, brief="Rolling for " + CURRENCY_NAME + " is no joke! '!roll [1-4] [2-unlimited]'")
    async def roll(ctx, left: int, right: int):
        config.read('Other/config.ini')
        spam = config.getboolean('features', 'notneededmsg')

        if right == 1:
            if spam:
                await ctx.send("Yes {0} ".format(ctx.message.author.mention) + "lets roll some 1 sided dice... "
                                                                               "how the fuck dat work?")
            print("Someone just tried to roll some 1 sided dice")
        else:
            if left == 1:
                dice_sides = random.randint(1, right)
                await ctx.send(ctx.message.author.mention + ' Rolled:   ' + str(dice_sides) + " / " + str(right))
                print("Just rolled 1 dice with " + str(right) + " sides. Got a total of " + str(dice_sides))
            elif left == 2:
                dice_sides = random.randint(1, right)
                dice_sides_2 = random.randint(1, right)
                total = dice_sides + dice_sides_2
                await ctx.send(ctx.message.author.mention + ' Rolled:   ' + str(dice_sides) + " / " + str(right) + "    "
                              + str(dice_sides_2) + " / " + str(right) + "    " + "Total:   " + str(total))
                print("Just rolled 2 dice with " + str(right) + " sides. Got a total of " + str(total))
            elif left == 3:
                dice_sides = random.randint(1, right)
                dice_sides_2 = random.randint(1, right)
                dice_sides_3 = random.randint(1, right)
                total = dice_sides + dice_sides_2 + dice_sides_3
                await ctx.send(ctx.message.author.mention + ' Rolled:   ' + str(dice_sides) + " / " + str(right) + "    "
                              + str(dice_sides_2) + " / " + str(right) + "    " + str(dice_sides_3) + " / " + str(right)
                              + "    " + "Total:   " + str(total))
                print("Just rolled 3 dice with " + str(right) + " sides. Got a total of " + str(total))
            elif left == 4:
                dice_sides = random.randint(1, right)
                dice_sides_2 = random.randint(1, right)
                dice_sides_3 = random.randint(1, right)
                dice_sides_4 = random.randint(1, right)
                total = dice_sides + dice_sides_2 + dice_sides_3 + dice_sides_4
                await ctx.send(ctx.message.author.mention + ' rolled:   ' + str(dice_sides) + " / " + str(right) + "    "
                              + str(dice_sides_2) + " / " + str(right) + "    " + str(dice_sides_3) + " / " + str(right)
                              + "    " + str(dice_sides_4) + " / " + str(right) + "    " + "Total:   " + str(total))
                print("Just rolled 4 dice with " + str(right) + " sides. Got a total of " + str(total))
            else:
                print("They fucked up")
                if spam:
                    await ctx.send(
                        "idk wtf you did, but fix it because it ain't right. Here I will help '!roll [1-4] [2-1000000]'")
                else:
                    await ctx.send("Command syntax error")
...
# insult api randome insult as user
...
# simplified blackjack game to earn currency
...
# Change log
    ...
    async def changes(ctx):
        ...
        embed.set_author(name="Change Log 4.2 (Worked on next command and removed/added little things to the voice "
                              "commands)")
        embed.add_field(name='Summary', value="Next command had a large amount of un-needed code so I removed that "
                                              "along with fixed how the code flows within the next command, added some "
                                              "dsicord msgs and removed/added some console print-outs for the voice "
                                              "commands to make it better for the bot owner to understand what "
                                              "was going on"
                        , inline=False)
        embed.add_field(name='Change', value="The next command is now where I would like. More optimization is "
                                             "going to be done to the voice commands along with the rest of the bot. "
                                             "The next command did not need to stop then move then play the next queued "
                                             "song, it only needed to stop the song so the after lambda function "
                                             "would call the 'check_queue' function",
                        inline=False)
        embed.add_field(name='Known Issues', value="The on_member_join to autorole someone the default role you have "
                                                   "set in the config does not work properly. A fix for this is being "
                                                   "looked into now",
                        inline=False)
        embed.add_field(name='Total lines', value="was: 2075 | is: 2066 ", inline=False)
