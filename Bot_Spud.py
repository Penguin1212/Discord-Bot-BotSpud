...
# When a new member joins what is done

    @bot.event
    async def on_member_join(member):
        role = get(member.server.roles, name=AUTO_ROLE)
        await member.add_role(member, role)
...
# this will play music from a youtube or soundcloud url
...
# this will pause the currently playing music
    ...
    @bot.command(pass_context=True, brief="Makes the bot leave your channel", aliases=['pa'])
    async def pause(ctx):
        config.read('Other/config.ini')
        Vstate = config.getboolean('features', 'discordvoice')
        spam = config.getboolean('features', 'notneededmsg')
...
# this will resume the currently paused music
    ...
    @bot.command(pass_context=True, brief="Makes the bot leave your channel", aliases=['res'])
...
# This will stop the currently playing music
    ...
    @bot.command(pass_context=True, brief="Makes the bot leave your channel", aliases=['st'])
    async def stop(ctx):
        global queues
        queues.clear()
        config.read('Other/config.ini')
...
# Queues up a song (in testing right now)

    @bot.command(pass_context=True, brief="Makes the bot leave your channel", aliases=['qu', 'q'])
...
# This will skip to next song (In testing right now)
    ...
    @bot.command(pass_context=True, aliases=['skip', 'sk', 'nex'])
    async def next(ctx):
        config.read('Other/config.ini')
        Vstate = config.getboolean('features', 'discordvoice')
        spam = config.getboolean('features', 'notneededmsg')
...
# Changes volume (in testing right now) *Disabled*
    ...
    @bot.command(pass_context=True, brief="Makes the bot leave your channel", aliases=['vol'])
...
time.sleep(2)
if amounts_infile is True and STARTOP is True:
...
elif amounts_infile is False or STARTOP is False:
    system('cls')
    main()
